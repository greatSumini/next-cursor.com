...
// 회원가입 제한 사유 레이블 매핑
const REGISTRATION_RESTRICTIONS_LABELS: Record<string, string> = {
  false_info: "허위 정보 기재",
  previous_violation: "이전 약관 위반 이력",
  minor_no_consent: "미성년자 법정대리인 동의 미확인",
  no_verification: "실명 미인증",
  illegal_usage: "부정 이용",
};

// 광고 채널 레이블 매핑
const ADVERTISING_CHANNELS_LABELS: Record<string, string> = {
  email: "이메일",
  sms: "SMS",
  push: "앱 푸시 알림",
  inapp: "앱/웹 내 알림",
...
// 결제 수단 레이블 매핑
const PAYMENT_OPTIONS_LABELS: Record<string, string> = {
  card: "카드결제",
  phone: "ARS, 휴대전화",
  bank_transfer: "계좌이체",
  e_money: "전자화폐",
...
// 관할법원 레이블 매핑
const JURISDICTION_COURTS_LABELS: Record<string, string> = {
  company_location: "회사 소재지 관할법원",
  seoul_central: "서울중앙지방법원",
  user_location: "이용자 주소지 관할법원",
};

export function generateTermsOfService(data: TermsOfServiceFormData): string {
  const {
    serviceProvider,
    serviceType,
  ...
  // 회원가입 제한 사유 문자열 생성
  const registrationRestrictionsStr = registrationRestrictions
    .map((restriction) => REGISTRATION_RESTRICTIONS_LABELS[restriction])
    .concat(
      customRegistrationRestriction ? [customRegistrationRestriction] : []
    )
    .filter(Boolean)
    .join(", ");

  // 광고 채널 문자열 생성
  const advertisingChannelsStr = advertisingChannels
    .map((channel) => ADVERTISING_CHANNELS_LABELS[channel])
    .concat(customAdvertisingChannel ? [customAdvertisingChannel] : [])
    .filter(Boolean)
    .join(", ");

  // 결제 수단 문자열 생성
  const paymentOptionsStr = paymentOptions
    .map((option) => PAYMENT_OPTIONS_LABELS[option])
    .concat(customPaymentOption ? [customPaymentOption] : [])
    .filter(Boolean)
    .join(", ");

  // 관할법원 문자열 생성
  const jurisdictionCourtStr =
    JURISDICTION_COURTS_LABELS[jurisdictionCourt] || jurisdictionCourt;

  // 서비스이용약관 생성
  return `# ${serviceProvider.companyName} 서비스 이용약관

## 제1조 (목적)

이 약관은 ${
    serviceProvider.companyName
  }(이하 "회사"라 함)이 제공하는 서비스의 이용과 관련하여 회사와 이용자 간의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
3. 회사는 이용자에게 서비스를 제공함에 있어 관련 법령, 약관, 운영정책 및 공지사항 등에서 정한 바에 따라 ${
    serviceType === "free"
      ? "무료로 서비스를 제공합니다."
      : serviceType === "paid"
      ? "유료로 서비스를 제공합니다."
...
${
  allowsAdvertising
    ? `1. 회사는 회원이 서비스 이용 중 필요하다고 인정되는 다양한 정보를 공지사항이나 ${advertisingChannelsStr} 등의 방법으로 회원에게 제공할 수 있습니다.
...
${
  hasPointSystem
    ? `1. 회사는 서비스의 효율적 이용을 위해 포인트 제도를 운영합니다.
2. ${pointPolicies}
3. 포인트는 회원 탈퇴 시 자동으로 소멸됩니다.`
    : "회사는 현재 포인트 제도를 운영하지 않습니다."
}

${
  hasCouponSystem
    ? `4. 회사는 서비스 활성화를 위해 쿠폰 제도를 운영합니다.
5. ${couponPolicies}
6. 쿠폰은 발급 시 명시된 유효기간 내에만 사용 가능하며, 유효기간이 만료된 쿠폰은 자동으로 소멸됩니다.`
...
${
  allowsContentCreation
    ? `
## 제${serviceType === "free" ? "16" : "20"}조 (게시물의 관리)
...
## 제${serviceType === "free" ? "17" : "21"}조 (게시물의 저작권)
...
${
  allowsContentUsage
   ...
   ${
     allowsDerivativeWorks
       ? "1) 게시물을 변형하여 2차적 저작물을 작성하는 경우"
       : ""
   }
   ${
     allowsPromotionalUse
       ? "2) 미디어, 통신사 등을 통한 홍보 목적으로 사용하는 경우"
       : ""
   }
   ${
     allowsServiceImprovement
...
2. 회사와 회원 간 발생한 분쟁에 관한 소송은 ${jurisdictionCourtStr}의 관할로 합니다.
...
1. 이 약관은 ${effectiveDate}부터 시행합니다.`;
}
